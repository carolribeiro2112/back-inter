...
export default class PixService {
  ...
  async pay(key: string, user: Partial<User>) {
    ...
    const resquestingUser = await userRepository.findOne({
    ...
    if (payingUser?.wallet && payingUser.wallet < Number(keyDecoded.value)) {
      throw new AppError ('Saldo insuficiente para fazer o pagamento', 401)
    }

    if(!resquestingUser || !payingUser) {
      throw new AppError('Usuários da transação não encontrados, gere uma nova chave', 401)
    }

    resquestingUser.wallet = Number(resquestingUser?.wallet) + Number(keyDecoded.value);
    await userRepository.save(resquestingUser)
  ...
  async transactions(user: Partial<User>) {
    ...
    const pixReceived = await (await pixRepository.find({
      where: {requestingUser: user.id, status: 'closed'},
      relations: ['payingUser']
    }))

    const pixPaying = await pixRepository.find({
      where: {payingUser: user.id, status:'closed'},
